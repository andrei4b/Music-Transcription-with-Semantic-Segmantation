...
from MAESTRO_ProcessLabel import ProcessLabel
import argparse
import numpy as np
...
def main(args):
    ...
    train_folders = ["2017"]
    test_folders  = []
    train_folders = [os.path.join(args.MAESTRO_path, ff) for ff in train_folders]
    test_folders  = [os.path.join(args.MAESTRO_path, ff) for ff in test_folders]
    ...
    if args.save_path is None:
        args.save_path = os.path.join(args.MAESTRO_path, "./features")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Program to process MAESTRO features for training and testing.")
    parser.add_argument("--MAESTRO-path", 
                        help="Path to your downloaded MAESTRO folder path. (default: %(default)s)",
                        type=str, default="./")
    ...
    parser.add_argument("--save-path",
                        help="Path for saving the output files (default: <MAESTRO>/features)",
                        type=str)
